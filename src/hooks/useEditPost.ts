...
import { EditPostData, Post as PostResponse } from "../types/Post.interfaces";
...
const useEditPost = () => {
  ...
  const editPost = async (postId : number, postData : EditPostData) : Promise<PostResponse> => {
    setLoading(true);
    setError(null);
    setSuccess(null);
    try {
      const response = await axiosInstance.put<PostResponse, AxiosResponse<PostResponse>, EditPostData>(
        `${API_URL.post}/${postId}`,
        postData
      );
